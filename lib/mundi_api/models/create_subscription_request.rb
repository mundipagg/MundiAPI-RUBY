...
module MundiApi
  class CreateSubscriptionRequest < BaseModel
    ...
    def self.from_hash(hash)
      ...
      pricing_scheme = CreatePricingSchemeRequest.from_hash(hash['pricing_scheme']) if hash['pricing_scheme']
      # Parameter is an array, so we need to iterate through it
      items = nil
      if hash['items'] != nil
        items = Array.new
        hash['items'].each{|structure| items << (CreateSubscriptionItemRequest.from_hash(structure) if structure)}
      end
      ...
      if hash['discounts'] != nil
        discounts = Array.new
        hash['discounts'].each{|structure| discounts << (CreateDiscountRequest.from_hash(structure) if structure)}
      end
