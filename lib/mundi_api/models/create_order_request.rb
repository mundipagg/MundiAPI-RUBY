...
module MundiApi
  class CreateOrderRequest < BaseModel
    ...
    def self.from_hash(hash)
      ...
      if hash['items'] != nil
        items = Array.new
        hash['items'].each{|structure| items << (CreateOrderItemRequest.from_hash(structure) if structure)}
      end
      customer = CreateCustomerRequest.from_hash(hash['customer']) if hash['customer']
      # Parameter is an array, so we need to iterate through it
      payments = nil
      if hash['payments'] != nil
        payments = Array.new
        hash['payments'].each{|structure| payments << (CreatePaymentRequest.from_hash(structure) if structure)}
      end
      code = hash['code']
